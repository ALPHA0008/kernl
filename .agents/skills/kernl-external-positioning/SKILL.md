...
- Internal docs style, README/AGENTS.md maintenance → `kernl-docs-and-writing`.
...
|---|---|---|---|
| C1 | Compile-don't-search ("agents are compilers") | Tribal knowledge is compiled ONCE into structured executable skills; the runtime reads compiled output instead of searching raw documents at query time | Thesis: `AGENTS.md:10`; pipeline DAG: `backend/engine/graph.py:33-80` (load → chunk → 5-way parallel extraction → graph → metadata → skills → evidence → confidence → write) |
...
| Thesis sentence | `sed -n 10p AGENTS.md` |
