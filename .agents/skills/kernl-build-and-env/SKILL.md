...
**Trap:** the root `AGENTS.md` describes an older layout (`backend/main.py`, `backend/llm.py`, OpenAI SDK client, 5-table schema). The actual code is `backend/api.py`, `backend/core/llm.py` (httpx client), and a 7-table `backend/schema.sql` (as of 2026-07-07). When AGENTS.md and code disagree, the code wins.
...
- Do not write new code against the `openai` package (dead dep) or against AGENTS.md's described layout (stale).
