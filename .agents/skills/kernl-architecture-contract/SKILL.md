...
| The graph is compiled **without a checkpointer**: `workflow.compile()` | backend/engine/graph.py:85 | This is a known gap, not a design choice — see weak point W7 below. The `MemorySaver` claim in AGENTS.md ("State checkpointing: MemorySaver") is stale; `grep -rn MemorySaver backend/` returns nothing (as of 2026-07-07). |
...
Caveat you will trip on: `_load_db` selects the **newest by `compiled_at`**, not `is_current = true` (brain_agent.py:613-618) — normally identical, but they diverge after a manual rollback. Also AGENTS.md calls `compile_runs` append-only, yet `write_brain` updates the run row's status (write_brain.py:146-153) — treat AGENTS.md as aspirational there. See `kernl-debugging-playbook` before "fixing" either.
...
| W7 | **No checkpointing** — `workflow.compile()` with no checkpointer; a mid-pipeline crash loses all extraction work (a full compile is ~291 s on the reference run). AGENTS.md's "MemorySaver" claim is stale. | backend/engine/graph.py:85 | Compile is all-or-nothing; no resume, no mid-run inspection. Any retry re-pays the full LLM cost (partially offset by the in-process `_content_cache`, backend/core/llm.py:21). |
