...
| `d215501` | 2026-07-07 | Adds `.Codex/` skill library (this file's ancestors) | — |
...
|---|---|---|
| `AGENTS.md` | Documents the pre-`2ca7f83` tree: `backend/main.py`, `backend/llm.py`, `backend/graph/` (`AGENTS.md:19-26`); mandates `AsyncOpenAI` (`AGENTS.md:117`) though the client is now raw `httpx`. One full refactor stale. | `git grep -n "main.py\|graph/" AGENTS.md` |
...
- **Lesson:** big-bang commits rot satellites. After any large refactor, sweep: AGENTS.md, README, Dockerfile, `backend/tests/`, `scripts/`, and the frontend — grep for every renamed symbol and path. Doc-repair procedure: `kernl-docs-and-writing`.
...
- **Symptom/Trigger:** AGENTS.md-era docs reference a PRD, but `company_brain_PRD_v4.md` and `brand_alchemy_company_brain.html` are nowhere in the working tree.
...
- [ ] Is it a large multi-subsystem refactor? -> E5: plan the satellite sweep (AGENTS.md, README, Dockerfile, tests, scripts, frontend) into the same change.
...
| AGENTS.md staleness (E5) | `git grep -n "main.py" AGENTS.md` |
