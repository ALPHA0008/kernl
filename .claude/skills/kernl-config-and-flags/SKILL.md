...
`backend/.env.example` (as of 2026-07-08) shows `VLLM_BASE_URL=http://<MI300X_IP>:8000/v1` — the `/v1` suffix is from a pre-refactor OpenAI-SDK era and will 404 against the current gateway code; it omits `VLLM_API_KEY` entirely; and its `COMPANY_ID` is dead (above). The root `CLAUDE.md` env section is stale in the same way.
...
| 8000 | `backend/.env.example:1`, root `CLAUDE.md` | pre-refactor vLLM-direct era; nothing current listens there |
