...
| **Docs-only** | `CLAUDE.md`, `README.md`, `docs/`, `.claude/skills/` | No eval. But: must not contradict code (see section 5 for a live contradiction handled correctly), must date-stamp volatile claims, and must never contain credential values. |
...
`CLAUDE.md:417` (Critical Rules #6) says: **"`compile_runs` table is append-only — never update rows, only insert status."**
...
| CLAUDE.md append-only rule | `grep -n "append-only" CLAUDE.md` |
