"""Seed verification: the authored rivanly-inc bundle [synthetic] must pass
its entire migrated golden set through the replay engine, 100%.

This is the V1 acceptance suite -- if a policy/priority interaction is wrong,
it fails here with the exact case and outcome diff.

Deterministic: no LLM, no DB, no network. Run directly or via pytest:
    py -3 backend/tests/test_seed_rivanly.py
"""
...
def test_bundle_builds_and_evidence_is_verified():
    bundle = build_bundle()
    assert len(bundle.policies) == 22
    assert len(bundle.workflows) == 9
    # every policy carries verified evidence with a real span
...
def test_golden_cases_pass_100_percent():
    ...
    run = engine.run(company_id="rivanly-inc", cases=cases, candidate=bundle)
